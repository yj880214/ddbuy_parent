...
<head>
    ...
    <script type="text/javascript">
        $(function () {
            //*********************************查询区域：显示并自动分页************************************
            $("#dg").datagrid({
                url:"/getTbContentCategory", //服务器请求（后台控制器的实现请求）
                pagination:true,     //开启分页
                pageList:[3,4,5,8,10], //分页设置
                pageSize:4,          //每页大小（行数）相当于自定义total
                toolbar:"#ToolBar",  //工具栏,toorbar 注意全部小写
                fitColumns:true,    //表格和页面自适应（列/宽度）
                // singleSelect:true,
                columns:[[
                    {field:'ck',checkbox:true},  //设置复选框
                    {field:'id',title:'广告类型的编号',width:100,align:"center"},
                    {field:'name',title:'广告类型的位置',width:100,align:"center"},
                    {field:'opt',title:'操作',width:100,align:"center",
                        formatter: function(value,row,index){  // 字段的值，此行数据，行索引
                            ...
                            return "<a href='javascript:delSingle("+row.id+")'>删除</a>  | <a href='javascript:openStreetDialog("+row.id+")'>查看广告</a>";
        ...
        function SaveDialog() {
            // alert("我要添加数据！");
            $('#addDialogForm').form('submit', {   //表单提交异步操作：添加
        ...
        function openStreetDialog(s) {
            ...
            $("#streetDg").datagrid({
                url:"${pageContext.request.contextPath}/getStreet?did="+s, //服务器请求（后台控制器的实现请求）
                pagination:true,     //开启分页
...
<%--修改对话框中设置“保存”、“取消”按钮--%>
  ...
  <%--街道对话框--buttons="#AddDialogButtons"--%>
